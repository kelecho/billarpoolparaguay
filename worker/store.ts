...
const TABLES = [
  ...
  { name: 'tournaments', key: ['id'], columns: ['name', 'date', 'venue', 'discipline', 'category', 'race_to', 'draw', 'format', 'qualifiers', 'banner'], order: 'rowid' },
  { name: 'registrations', key: ['tournament_id', 'player_id'], columns: ['position', 'seed'], order: 'tournament_id, position' },
  { name: 'matches', key: ['tournament_id', 'id'], columns: ['round', 'idx', 'bracket', 'score_a', 'score_b', 'table_name', 'time'], order: "tournament_id, CASE bracket WHEN 'P' THEN 1 WHEN 'F' THEN 2 ELSE 0 END, round, idx" },
...
function flatten(state: State): Rows {
  ...
  return {
    ...
    tournaments: state.tournaments.map(t => ({ id: t.id, name: t.name, date: t.date, venue: t.venue, discipline: t.discipline, category: t.category ?? null, race_to: t.raceTo ?? null, draw: t.fixture?.draw ?? null, format: t.format ?? null, qualifiers: t.qualifiers ?? null, banner: t.banner ?? null })),
...
export async function load(db: D1Database): Promise<{ state: State; version: number; tag: string }> {
  ...
  const state = {
    ...
    tournaments: tournaments.map(t => {
      ...
      return present({
        id: t.id, name: t.name, date: t.date, venue: t.venue, discipline: t.discipline, category: t.category, raceTo: t.race_to, format: t.format, qualifiers: t.qualifiers, banner: t.banner,
