...
const TABLES = [
  ...
  { name: 'tournaments', key: ['id'], columns: ['name', 'date', 'venue', 'discipline', 'category', 'race_to', 'draw', 'format', 'qualifiers', 'banner', 'third_place', 'final_rematch', 'league_rounds', 'open'], order: 'rowid' },
...
export async function load(db: D1Database): Promise<{ state: State; version: number; tag: string }> {
  ...
  const state = {
    ...
    tournaments: tournaments.map(t => {
      ...
      return present({
        id: t.id, name: t.name, date: t.date, venue: t.venue, discipline: t.discipline, category: t.category, raceTo: t.race_to, format: t.format, qualifiers: t.qualifiers, banner: t.banner, thirdPlace: t.third_place ? true : null, finalRematch: t.final_rematch ? true : null, leagueRounds: t.league_rounds, open: t.open ? true : null,
